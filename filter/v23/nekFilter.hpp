...
{ 
  ...
  applyFilterKernel = platform->device.buildKernel(fileName, kernelInfo, true);
...
occa::memory lowPassFilterSetup_new(std::string tag, mesh_t *mesh, 
  ...
  nrsCheck(filterWght < 0 || filterWght > 1,
           platform->comm.mpiComm,
           EXIT_FAILURE,
