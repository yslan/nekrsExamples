...
occa::kernel setBCScalarValueKernel;
occa::kernel scalarMaskCopyKernel;
...
void velRecycling::buildKernel(occa::properties kernelInfo)
{
  ...
  const std::string extension = ".okl";
  {
    ...
    kernelName = "setBCScalarValue";
    fileName = path + kernelName + extension;
    setBCScalarValueKernel = platform->device.buildKernel(fileName, kernelInfo, true);

    kernelName = "velRecyclingMaskCopy";
    fileName = path + kernelName + extension;
    maskCopyKernel = platform->device.buildKernel(fileName, kernelInfo, true);

    kernelName = "scalarRecyclingMaskCopy";
    fileName = path + kernelName + extension;
    scalarMaskCopyKernel = platform->device.buildKernel(fileName, kernelInfo, true);
  }
...
void velRecycling::copy_scalar(nrs_t *nrs, occa::memory o_wrk_, occa::memory o_fld_)
{
  nrsCheck(!setupCalled || !buildKernelCalled, MPI_COMM_SELF, EXIT_FAILURE,
           "%s\n", "called prior to tavg::setup()!");

  mesh_t *mesh = nrs->meshV; // only do V-mesh for not

  if (interp) {
    const dlong offset = o_Uint.length() / nrs->NVfields;
    interp->eval(1, nrs->fieldOffset, o_fld_, offset, o_Uint); // o_fld_ to o_Uint
    scalarMaskCopyKernel(interp->numPoints(), o_maskIds, o_Uint, o_wrk_); // o_Uint to o_wrk_
  } else {
    o_wrk_.copyFrom(o_fld_, nrs->fieldOffset);
    const dfloat zero = 0.0;
    setBCScalarValueKernel(mesh->Nelements, zero, bID, o_wrk, mesh->o_vmapM, mesh->o_EToB);
    oogs::startFinish(o_wrk, 1, nrs->fieldOffset, ogsDfloat, ogsAdd, ogs);
  }

  // Rescale or shift the values yourself!
}
